...
class FacebookHatefulMemesDataset(BaseDataset):
    ...
    def __init__(
        self, config: DatasetConfig, processor, prompts_file: Optional[str] = None
    ):
    ...
    def load_dataset(self) -> None:
        ...
        train_path = Path(self.config.data_path) / "fine_grained_labels" / "train.jsonl"
        ...
        if self.config.max_samples:
            ...
            data = random.sample(data, min(self.config.max_samples, len(data)))

        for item in data:
            img_path = Path(self.config.data_path) / item["img"]
            if img_path.exists():
                self.items.append(
                    {
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict, Dict, str, Dict]:
        """Get item with optional prompt variation."""
        if self.prompts:
            # Multi-prompt mode
            ...
            # Single prompt mode
            item = self.items[idx]
            prompt = "Is this image harmful? If yes, what group is targeted and how?"
            metadata = {}
