...
def custom_collate_fn_img(batch):
    ...
    padded_pixel_values = torch.zeros(
        (batch_size, num_patches, channels, max_height, max_width),
        dtype=pixel_values[0].dtype,
    )
    padded_pixel_attention_mask = torch.zeros(
        (batch_size, num_patches, max_height, max_width),
...
def qwen_custom_collate_fn(batch):
    ...
    padded_input_ids = torch.full(
        (batch_size, max_len), 151643, dtype=torch.long
    )  # Qwen padding token
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict, int]:
        ...
        message = {
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": "Is this meme harmful or not harmful? Please respond with exactly one word: either 'harmful' or 'not_harmful'.",
                },
            ],
        }
...
class MultiPromptMultiLabelMemeDatasetQwen(Dataset):
    def __init__(
        ...
        with open(
            "../results/extreme_positions_diagonal_15_per_quadrant.pkl", "rb"
        ) as f:
            extreme_pos = pickle.load(f)
        self.extreme_pos_ids = [
            pos[0]
            for pos in extreme_pos["top_right"]
            + extreme_pos["top_left"]
            + extreme_pos["bottom_right"]
            + extreme_pos["bottom_left"]
        ]
...
class US2016Dataset(Dataset):
    def __init__(
        ...
        self.extreme_pos_ids = get_persona_ids(
            "../results/left_and_right_40_personas.pkl"
        )
        self.id_to_pos = get_persona_id_to_pos_dict(
            "../results/left_and_right_40_personas.pkl"
        )
    ...
    def load_dataset(self, max_samples: Optional[int]) -> None:
        ...
        for _, row in data.iterrows():
            ...
            png_path = os.path.join(
                self.base_path, "images/imgur_US_2016", img_name + ".png"
            )
            jpeg_path = os.path.join(
                self.base_path, "images/imgur_US_2016", img_name + ".jpeg"
            )
...
class SinglePromptMemeDataset(Dataset):
    def __init__(
        self, base_path: str, processor, prompt: str, max_samples: Optional[int] = None
    ):
        self.base_path = base_path
        self.processor = processor
        self.prompt = prompt
